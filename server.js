...
app.use(cors({
    origin: (origin, cb) => {
        ...
        else cb(null, true); // allow all for now — restrict in production if needed
    },
...
app.get('/health', (req, res) => res.json({
    status: 'ok', version: '2.4', name: 'La Mafia Spy', database: 'supabase'
}));
...
app.post('/search', requireAuth, async (req, res) => {
    const { keywords = '', min_copies = 1, country = 'BR', ad_type = 'all', max_results = 50, active_status = 'all' } = req.body;
    if (!keywords.trim()) return res.status(400).json({ detail: 'Keywords cannot be empty.' });

    const searchUrl = `https://www.facebook.com/ads/library/?active_status=${active_status}&ad_type=${ad_type}&country=${country}&q=${encodeURIComponent(keywords)}&search_type=keyword_unordered&media_type=all`;

    try {
        const results = await scrapeAds({ keywords, min_copies, country, ad_type, max_results: Math.min(max_results, 200), active_status });
...
app.post('/transcribe', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // If already PT, return original as "translation"
...
// ─── ALERTS (Protected) ──────────────────────────────────
app.get('/alerts', requireAuth, async (req, res) => res.json(await getAlerts(req.user.id)));
...
app.get('/libraries', requireAuth, async (req, res) => res.json(await getLibraries(req.user.id)));
...
app.post('/libraries/:id/analyze', requireAuth, async (req, res) => {
    ...
    try {
        const ads = await scrapePageLibrary({ pageUrl: lib.url, country: lib.country || 'BR', max_results: 100 });
...
// ─── START ───────────────────────────────────────────────
app.listen(PORT, () => {
    console.log(`\n👁️  La Mafia Spy — v2.4 (Supabase + Public Swipe File)`);
