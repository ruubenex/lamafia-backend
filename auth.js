...
export async function registerUser({ name, email, password }) {
    try {
        // Use admin API to create user AND confirm email in one step
        // This bypasses the "email confirmation required" issue
        const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
            email,
            password,
            email_confirm: true,   // <-- confirms email immediately, no confirmation email needed
            user_metadata: { name }
        });
        ...
        // Now sign in to get a real access_token (admin.createUser doesn't return one)
        const { data: signInData, error: signInError } = await supabase.auth.signInWithPassword({ email, password });
        if (signInError || !signInData.session) {
            return { ok: false, msg: 'Conta criada, mas erro ao fazer login automático. Faça login manualmente.' };
        }
